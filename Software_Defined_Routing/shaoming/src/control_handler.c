...
bool control_recv_hook(int sock_index)
{
    ...
    if(payload_len != 0){
        ...
        //*** hear write code to get payload. we need to use control_code to decide which struct we use to save data.
    }
    ...
    switch(control_code){
        ...
        case 1: init_response(sock_index, cntrl_payload);
                printf("**end init_response**\n");
                init_table(cntrl_payload);
                printf("**end init_table**\n");
                init_list();
                printf("**end init_list**\n");
                //print_list();
                break;
        ...
        case 3: update_response(sock_index);
                printf("**end update_response**\n");
                update_topology_table(cntrl_payload);
                printf("**end update_topology_table**\n");
                break;
        case 4: crash_response(sock_index);
                printf("**end crash_response**\n");
                crash_router();
                printf("**end crash_router**\n");
                break;
        /* .......
            .........
           ....... 
